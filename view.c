#include "view.h"
#include "controller.h" // Controller tipi için gerekli
#include <gtk/gtk.h>
#include <string.h>
#include <stdlib.h>
...
int view_add_terminal(View *view, const char *title) {
    ...
    int terminal_index = view->terminal_count;
    int terminal_id = view->next_terminal_id++;
    ...
    view->terminal_views[terminal_index] = gtk_text_view_new();
    gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(view->terminal_views[terminal_index]), GTK_WRAP_WORD_CHAR);
    gtk_text_view_set_monospace(GTK_TEXT_VIEW(view->terminal_views[terminal_index]), TRUE);
    gtk_text_view_set_editable(GTK_TEXT_VIEW(view->terminal_views[terminal_index]), FALSE);
    gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(terminal_scroll), view->terminal_views[terminal_index]);
    
    view->terminal_buffers[terminal_index] = gtk_text_view_get_buffer(GTK_TEXT_VIEW(view->terminal_views[terminal_index]));
    ...
    g_object_set_data(G_OBJECT(view->input_entries[terminal_index]), "terminal_id", GINT_TO_POINTER(terminal_id));
    g_signal_connect(view->input_entries[terminal_index], "activate", G_CALLBACK(on_command_entry_activated), view);
    
    // Terminal ID'sini kaydet
    view->terminal_ids[terminal_index] = terminal_id;
    ...
    gtk_notebook_set_current_page(GTK_NOTEBOOK(view->terminal_notebook), terminal_index);
    view->active_terminal = terminal_index;
    view->terminal_count++;
    ...
    gtk_text_buffer_insert(view->terminal_buffers[terminal_index], &iter, 
                          ...
                          "Terminali kapatmak için exit yazabilirsiniz.\n\n", -1);
    
    return terminal_id;
}

// Terminal indeksini ID'den bulur
int view_find_terminal_index(View *view, int terminal_id) {
    if (!view) return -1;
    
    for (int i = 0; i < view->terminal_count; i++) {
        if (view->terminal_ids[i] == terminal_id) {
            return i;
        }
    }
...
int view_close_terminal(View *view, int terminal_id) {
    ...
    // Sekmeyi kaldır
    gtk_notebook_remove_page(GTK_NOTEBOOK(view->terminal_notebook), terminal_index);
    
    // Terminal verilerini kaydır
    for (int i = terminal_index; i < view->terminal_count - 1; i++) {
        ...
        view->terminal_ids[i] = view->terminal_ids[i + 1];
    }
...
int view_clear_terminal(View *view, int terminal_id) {
    if (!view) {
        return -1;
    }
    ...
    GtkTextBuffer *text_buffer = view->terminal_buffers[terminal_index];
...
void view_cleanup(View *view) {
    ...
    if (view->terminal_ids) g_free(view->terminal_ids);
...
void on_command_entry_activated(GtkEntry *entry, gpointer user_data) {
    ...
    if (strcmp(command, "exit") == 0) {
        ...
        if (view->terminal_count <= 1) {
            // GTK4 modern dialog kullanımı - GtkAlertDialog* tipini GtkWidget* olarak kullanmak yerine doğrudan kullan
            GtkAlertDialog *dialog = gtk_alert_dialog_new("En az bir terminal açık kalmalıdır.");
            gtk_alert_dialog_show(dialog, GTK_WINDOW(view->window));
            g_object_unref(dialog);
        } else {
            // Terminali kapat
            view_close_terminal(view, terminal_id);
            return; // Komut işlemeye devam etme
        }
    } else {
        // Komut işleme mantığı controller'a taşındı
        // Burada sadece controller'a komut gönderiyoruz
        extern int controller_process_input(Controller *controller, int terminal_id, const char *input);
        extern Controller *g_controller; // Global controller referansı
        
        if (g_controller) {
            controller_process_input(g_controller, terminal_id, command);
        }
    }
...
void on_new_terminal_clicked(GtkButton *button, gpointer user_data) {
    ...
    char title[32];
    snprintf(title, sizeof(title), "Terminal %d", view->next_terminal_id + 1);
...
void on_close_terminal_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (data->view->terminal_count <= 1) {
        // GTK4 modern dialog kullanımı - GtkAlertDialog* tipini GtkWidget* olarak kullanmak yerine doğrudan kullan
        GtkAlertDialog *dialog = gtk_alert_dialog_new("En az bir terminal açık kalmalıdır.");
        gtk_alert_dialog_show(dialog, GTK_WINDOW(data->view->window));
        g_object_unref(dialog);
        return;
    }
...
gboolean update_messages_timeout(gpointer user_data) {
    ...
    if (result > 0 && strlen(last_message) > 0) {
        ...
        if (sscanf(last_message, "T%d:%[^\n]", &sender_terminal, message_content) == 2) {
            ...
            // Format uyumsuz, ham mesajı göster
            view_update_message_panel(view, last_message, NULL);
        }
    }
